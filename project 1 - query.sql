...
CREATE TABLE remarks (
    ...
    loan_amount VARCHAR(200),
    cibil_score INT,
    cibil_score_status VARCHAR(200)
);
select * from remarks;
describe customer_det;
drop table customer_det;
describe  remarks_updating_details;
show triggers;
delimiter //
create trigger trigger1 after insert on customer_det
for each row
begin if new.cibil_score is not null then

if new.cibil_Score > 900
then insert into remarks(loan_id,loan_amount,cibil_score,cibil_score_status) 
values(new.loan_id,new.loan_amount,new.cibil_score,'high cibil score');
elseif new.cibil_Score > 750
then insert into remarks (loan_id,loan_amount,cibil_score,cibil_score_status) 
values(new.loan_id,new.loan_amount,new.cibil_score,'no penalty score');
elseif new.cibil_Score > 250
then insert into remarks(loan_id,loan_amount,cibil_score,cibil_score_status) 
values(new.loan_id,new.loan_amount,new.cibil_score,'penalty customer');
elseif new.cibil_Score <=0
then insert into remarks (loan_id,loan_amount,cibil_score,cibil_score_status)
 values(new.loan_id,new.loan_amount,new.cibil_score,'reject customer');
end if;
end if;
end //
 delimiter ;
 ...
 describe remarks_updating_details;
 delimiter //
 create trigger trigger2 before insert on customer_det for each row
begin 
if isnull(new.loan_Amount) then
 set new.loan_Amount= 'still processing';
end if;
end //
delimiter ;
drop trigger mail_check;
drop trigger age_check;
drop trigger check_community;
drop trigger salary_credit;
show triggers;
...
select * from customer_det; 
select *  from remarks;
select * from loan_cibilscore_det;
 -- create table loan_cibilscore_det as
SELECT 
    customer_det.loan_id,
    customer_det.customer_id,
    customer_det.loan_amount,
    customer_det.loan_amount_term,
    customer_det.cibil_score,
    remarks.cibil_score_status
FROM
    customer_det
        INNER JOIN
    remarks ON customer_det.loan_id = remarks.loan_id;
select * from customer_det;
select * from customer_det;
...
drop table loan_cibilscore_det;
drop table customer_det;
select customer_name,right(customer_name,4) as lastname from customer_det where customer_name like'%____';
select * from loan_dataset;
-- create table loan_dataset as
 SELECT 
    customer_det.customer_id,
    customer_det.customer_name,
    customer_det.age,
    customer_det.gender,
    customer_det.married,
    customer_det.loan_id,
    customer_det.self_employed,
    customer_det.region_id,
    loan_cibilscore_det.loan_amount,
    loan_cibilscore_det.loan_amount_term,
    loan_cibilscore_det.cibil_score,
    loan_cibilscore_det.cibil_score_status,
    customer_income.applicantincome,
    customer_income.coapplicantincome,
    customer_income.property_area,
    customer_income.loan_status,
    region_info.region,
    country_state.postal_code,
    country_state.segment,
    country_state.state
FROM
    loan_cibilscore_det
        INNER JOIN
    customer_det ON loan_cibilscore_det.customer_id = customer_det.customer_id
        INNER JOIN
    country_state ON country_state.customer_id = customer_det.customer_id
	right join
    region_info ON region_info.region_id = country_state.region_id
        INNER JOIN
    customer_income ON customer_det.customer_id = customer_income.customer_id;
    
    select * from loan_dataset;
    update loan_dataset set loan_amount=0 where loan_amount='still processing';
    
    select * from country_state right join region_info on country_state.region_id=
    region_info.region_id;
    select  * from low_class;
    create table low_class as
    select *,case
    when applicantincome > 15000
    then "A grade customer" 
    when applicantincome > 9000
    then "B grade customer" 
    when applicantincome > 5000
    then "middle class customer" 
    when applicantincome < 5000
    then "low class customer"
    end as customer_criteria from loan_dataset;
    drop table low_class;
    describe interest_loan_data;
  select * from  interest_loan_data;
    create table interest_loan_data
    select *,case when applicantincome <5000 and property_area='rural' and  customer_criteria='low class customer'
    then 5 
    when applicantincome <5000 and property_area='semiurban' and  customer_criteria='low class customer'
    then 3.5
    when applicantincome <5000 and property_area='urban' and  customer_criteria='low class customer'
    then 3 
    when applicantincome <5000 and property_area='semirural' and  customer_criteria='low class customer'
    then 3.5
    when applicantincome >5000 
    then 7 end as  interest  from low_class;
    
    drop table interest_loan_data;
      
    select * from loan_amount_interested;
    
    -- create table loan_amount_interested
    -- as 
    select *,cast(loan_amount*(interest/100)*30 as float) as  
    monthly_interest_amount ,cast(loan_amount*(interest/100)*(12*30) as  float) 
    as annual_income_amount from 
    interest_loan_data;
    select * from interest_loan_data;
    describe interest_loan_data;
    update interest_loan_data set loan_amount=0 where loan_amount='still processing';
select 
...
FROM
    ...
    delimiter ;
    delimiter //
    create procedure show_alldata()
    begin
    
    select state,count(customer_id) ,sum(loan_amount),sum(applicantincome)from loan_dataset group by state;
    end //
    ...
    delete from loan_amount_interested where cibil_score<200;
    
    
    select customer_name,right(customer_name,4) as lastname from loan_dataset where  customer_name like'%____';
    drop procedure show_alldata;
    update loan_dataset set gender='Female' where customer_id in ('IP43006','IP43016');
select * from loan_dataset;